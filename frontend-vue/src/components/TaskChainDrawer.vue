      ...
      <aside class="chain-drawer" role="dialog" aria-modal="true" aria-label="任务全链路详情">
        ...
        <div v-else-if="data" class="chain-content">
          ...
          <section v-if="data.risk_card" class="chain-section">
            <div class="section-title"><div><span>RISK CARD</span><h3>班前风险提示卡</h3></div><a :href="data.links.worker">进入班前风险分析 ↗</a></div>
          ...
          <section class="chain-section">
            <div class="section-title"><div><span>AUDIT</span><h3>关联方案控制项</h3></div><a v-if="data.task.audit_run_id" :href="data.links.audit">查看方案审查 ↗</a></div>
            ...
            <p v-else class="chain-empty">该任务暂未关联有效方案控制项。</p>
          </section>
...
.chain-map { grid-template-columns: repeat(4, 1fr); }
</style>
