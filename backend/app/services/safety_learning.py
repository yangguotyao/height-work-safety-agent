...
class QuizService:
    ...
    def create(
        self, *, worker_ref: str, task_id: str | None, scene: str | None
    ) -> dict[str, Any]:
        task = self.worker_repository.get_task(task_id) if task_id else None
        if task and task.get("worker_ref") and task["worker_ref"] != worker_ref:
            raise ValueError("该任务不属于当前工人标识")
        selected_scene = scene or self.bank.infer_scene(task)
        wrong_ids = self.learning_repository.active_wrong_question_ids(worker_ref)
        answered_ids = self.learning_repository.answered_question_ids(worker_ref)
        questions = self.bank.select(
            selected_scene, preferred_ids=wrong_ids, excluded_ids=answered_ids
        )
        attempt = self.learning_repository.create_quiz(
            worker_ref, task_id, selected_scene, [item["id"] for item in questions]
        )
        return {
            "id": attempt["id"],
            "worker_ref": worker_ref,
            ...
            "scene_name": self.bank.scene_catalog[selected_scene]["name"],
            "status": attempt["status"],
            "questions": [self.bank.public(item) for item in questions],
            "started_at": attempt["started_at"],
        }

    def get(self, attempt_id: str) -> dict[str, Any]:
        attempt = self.learning_repository.get_quiz(attempt_id)
        return {
            "id": attempt["id"],
            "worker_ref": attempt["worker_ref"],
            "task_id": attempt["task_id"],
            "scene": attempt["scene"],
            "scene_name": self.bank.scene_catalog[attempt["scene"]]["name"],
            "status": attempt["status"],
            "questions": [
                self.bank.public(self.bank.get(question_id))
                for question_id in attempt["question_ids"]
            ],
            "started_at": attempt["started_at"],
        }

    def submit(self, attempt_id: str, answers: list[dict[str, str]]) -> dict[str, Any]:
        attempt = self.learning_repository.get_quiz(attempt_id)
        if attempt["status"] != "in_progress":
            raise ValueError("该测验已经提交，不能重复作答")
        expected_ids = attempt["question_ids"]
        answer_map = {item["question_id"]: item["answer"] for item in answers}
        if len(answer_map) != len(answers) or set(answer_map) != set(expected_ids):
            raise ValueError("必须且只能提交本次测验的5道题答案")
        stored_answers: list[tuple[str, str, bool]] = []
        results: list[dict[str, Any]] = []
        for question_id in expected_ids:
            question = self.bank.get(question_id)
            submitted = answer_map[question_id]
            option_keys = {str(option["key"]) for option in question["options"]}
            if submitted not in option_keys:
                raise ValueError(f"题目 {question_id} 的答案选项无效")
            correct = submitted == question["answer"]
            stored_answers.append((question_id, submitted, correct))
            results.append(
                {
        ...
        self.learning_repository.submit_quiz(
            attempt_id, attempt["worker_ref"], stored_answers
        )
        return {
            "id": attempt_id,
            "worker_ref": attempt["worker_ref"],
    ...
    def learning_record(self, worker_ref: str) -> dict[str, Any]:
        record = self.learning_repository.learning_record(worker_ref)
        wrong_ids = self.learning_repository.active_wrong_question_ids(worker_ref)
        wrong_questions = [self.bank.get(item_id) for item_id in wrong_ids]
        recommendations = wrong_questions[:5]
        return {
            "worker_ref": worker_ref,
            **record,
            "active_wrong_count": len(wrong_questions),
            "wrong_questions": [self.bank.public(item) for item in wrong_questions],
            "recommendations": [
                {**self.bank.public(item), "reason": "历史错题，建议再次练习"}
